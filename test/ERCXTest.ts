...
describe("ERCXTest", function () {
  async function initialize() {
    ...
    return { contract, owner, delegatee, borrower, rentalContractMock, expires, fastForwardYear };
  }

  it("Operator is not owner or approved", async function () {
    const { contract, borrower } = await loadFixture(initialize);

    await expect(contract.connect(borrower).setUser(1, borrower.address, 0, false)).to.be.revertedWith(
  ...
  it("User cannot be zero address", async function () {
    ...
    await expect(contract.setUser(1, ethers.constants.AddressZero, 0, false)).to.be.revertedWith(
      "ERCX: set user to zero address"
    );
  });
  ...
  it("Cannot set user if NFT is borrowed", async function () {
    const { contract, delegatee, borrower, rentalContractMock, expires } = await loadFixture(initialize);
  ...
  it("Can delegate and redelegate user", async function () {
    ...
    await expect(contract.setUser(1, owner.address, expires, false)).to.emit(contract, "UpdateUser").withArgs(
      1, owner.address, expires, false
    );
  ...
  it("Can set user after borrow expires", async function () {
    const { contract, delegatee, borrower, expires, fastForwardYear } = await loadFixture(initialize);

    await expect(contract.setUser(1, borrower.address, expires, true)).to.emit(contract, "UpdateUser").withArgs(
      1, borrower.address, expires, true
    );
    await time.increaseTo(await time.latest() + fastForwardYear);
    await expect(contract.setUser(1, delegatee.address, 0, false)).to.emit(contract, "UpdateUser").withArgs(
      1, delegatee.address, 0, false
    );
  });
  ...
  it("Rental contract can set user", async function () {
    const { contract, borrower, rentalContractMock, expires } = await loadFixture(initialize);
