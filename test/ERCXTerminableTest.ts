...
describe("ERCXTerminableTest", function () {
  async function initialize() {
    ...
    return { contract, owner, delegatee, borrower, uint64MaxValue, expires, fastForwardYear };
  }
  ...
  it("Can reset borrow if owner mistakenly borrowed token to own wallet and set a long duration", async function () {
    ...
    await expect(contract.setBorrowTermination(1))
      ...
      .to.emit(contract, "AgreeToTerminateBorrow").withArgs(1, owner.address, false);

    expect(await contract.getBorrowTermination(1)).to.have.ordered.members([true, true]);

    await expect(contract.terminateBorrow(1))
      .to.emit(contract, "TerminateBorrow").withArgs(1, owner.address, owner.address, owner.address)
  ...
  it("Can reset borrow and set a new user if both parties agree", async function () {
    ...
    expect(await contract.getBorrowTermination(1)).to.have.ordered.members([true, false]);

    await expect(contract.connect(borrower).setBorrowTermination(1))
      .to.emit(contract, "AgreeToTerminateBorrow").withArgs(1, borrower.address, false);
  ...
  it("Agreed borrow terminations must be reset if userOf is changed", async function () {
    ...
    await expect(contract.setUser(1, delegatee.address, 0, false))
      .to.emit(contract, "UpdateUser").withArgs(1, delegatee.address, 0, false)
      .to.emit(contract, "ResetTerminationAgreements").withArgs(1);
