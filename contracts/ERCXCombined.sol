...
/**
 * @dev Implementation of complex ERCX contract with all extensions ---proposal_link--- with OpenZeppelin ERC721 version.
 */
contract ERCXCombined is IERCX, IERCXBalance, IERCXTerminable, IERCXEnumerable, ERC721 {
    /**
    ...
    function tokenOfUserByIndex(address user, uint256 index) public view virtual override returns (uint256){
        require(user != address(0), "ERCXEnumerable: address zero is not a valid owner");
        uint256[] memory balance = _userBalances[user];
        require(balance.length > 0 && index < balance.length, "ERCXEnumerable: owner index out of bounds");
        uint256 counter;
        unchecked {
